...
public class CubeRotation_MM : MonoBehaviour
{
  float playerRestriction;
    ...
    void Update ()
    {
      if(Input.GetKeyDown(KeyCode.F) && playerRestriction <= 0) // switched to WASD to deconflict with player moves
      {
        StartCoroutine(Rotate(Vector3.forward, 90, 1.0f));
        playerRestriction = 1.2f;
      }

      if(Input.GetKeyDown(KeyCode.H) && playerRestriction <= 0)
      {
        StartCoroutine(Rotate(Vector3.forward, -90, 1.0f));
        playerRestriction = 1.2f;
      }

      if(Input.GetKeyDown(KeyCode.T) && playerRestriction <= 0)
      {
        StartCoroutine(Rotate(Vector3.right, 90, 1.0f));
        playerRestriction = 1.2f;
      }

      if(Input.GetKeyDown(KeyCode.G) && playerRestriction <= 0)
      {
        StartCoroutine(Rotate(Vector3.right, -90, 1.0f));
        playerRestriction = 1.2f;
      }
